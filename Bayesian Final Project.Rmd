...
# Re-factor all categorical variables and covert them ·nto 1 and 0
birth <- birth %>% 
...
Thus, the prior model of Y would be: 

\begin{equation}
\begin{array}{lcrl} 
\text{data:} & \hspace{.05in} &  Y_i | \beta_0, \beta_1, ... , \beta_5, \sigma & \stackrel{ind}{\sim} N\left(\mu_i, \sigma^2\right) \;\; \text{ with } \;\; \mu_i = \beta_0 + \beta_1 X_{i1} + ... + \beta_5 X_{i5} \\
\text{priors:} & & \beta_0  &  \sim N\left(3200, 750^2 \right)  \\
                     & & \beta_1  & \sim N\left(108, 54^2 \right) \\
                     & & \beta_2  & \sim N\left(-250, 25^2 \right) \\
                     & & \beta_3  & \sim N\left(25, 2.5^2 \right) \\
                     & & \beta_4  & \sim N\left(0, 2.5^2 \right) \\
                     & & \beta_5  & \sim N\left(75, 12.5^2 \right) \\
                     & & \sigma & \sim \text{Exp}(0.003)  .\\
\end{array}
\end{equation}
...
```{r mcmc, out.width="90%", fig.align = 'center'}
